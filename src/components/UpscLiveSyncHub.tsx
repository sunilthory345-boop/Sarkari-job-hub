...
import { UpscLiveNotice, UpscSyncStatus, GovJob, AdmitCard, JobResult, AnswerKey } from '../types';

interface UpscLiveSyncHubProps {
  locale?: string;
...
export default function UpscLiveSyncHub({
  locale = 'en',
  onAddJob,
  onAddAdmitCard,
  onAddResult,
...
}: UpscLiveSyncHubProps) {
  const [notices, setNotices] = useState<UpscLiveNotice[]>([]);
  const [loading, setLoading] = useState<boolean>(true);
  const [syncing, setSyncing] = useState<boolean>(false);
  const [status, setStatus] = useState<UpscSyncStatus>({
    online: true,
  ...
  const [importedNoticeIds, setImportedNoticeIds] = useState<string[]>(() => {
    const saved = localStorage.getItem('sarkari_upsc_imported_ids');
  ...
  const fetchStatusAndFeed = async (isManualSync = false) => {
    ...
    try {
      // 1. Fetch Status
      const statusRes = await fetch('/api/upsc/status').catch(() => null);
      ...
      // 2. Fetch Feed
      const feedRes = await fetch('/api/upsc/live-feed').catch(() => null);
      if (feedRes && feedRes.ok) {
        ...
        if (fData.notices && Array.isArray(fData.notices)) {
          ...
          if (isManualSync) {
            triggerToast(
              locale === 'hi' 
                ? '⚡ https://www.upsc.gov.in/ से नवीनतम सूचनाएं सफलतापूर्वक सिंक हो गईं!' 
                : '⚡ Synced latest releases from https://www.upsc.gov.in/ successfully!'
            );
          }
        }
      }
    } catch (err) {
      console.error('Error syncing UPSC feed:', err);
    } finally {
      setLoading(false);
  ...
  const handleImportNotice = (notice: UpscLiveNotice) => {
    if (importedNoticeIds.includes(notice.id)) {
      triggerToast(locale === 'hi' ? 'यह यूपीएससी सूचना पहले से ही आपकी वेबसाइट पर लाइव है।' : 'This UPSC notice is already active on your website.');
      return;
    }
    ...
    try {
      if (notice.category === 'vacancy') {
        const jobToAdd: GovJob = notice.jobData || {
          id: `upsc-job-${Date.now()}`,
          title: notice.title,
          org: notice.org,
          category: 'UPSC',
          qualification: (notice.details?.qualification?.includes('12') ? '12th Pass' : 'Graduate') as any,
          ageLimit: '21-32 Years',
          salary: notice.details?.salary || 'Pay Level 10 (₹56,100 - ₹1,77,500)',
          fees: { General: '₹100', OBC: '₹100', SC_ST_Female: 'Exempted (₹0)' },
          totalPosts: notice.details?.posts || 500,
          ...
          importantDates: {
            ...
            examDate: notice.details?.examDate || 'Scheduled 2026/2027',
            admitCardRelease: '3 Weeks Before Exam'
          },
          selectionProcess: ['Preliminary Examination', 'Mains Examination', 'Personality Test (Interview)'],
          location: 'All India',
        ...
        const cardToAdd: AdmitCard = notice.admitCardData || {
          id: `upsc-admit-${Date.now()}`,
          title: notice.title,
          org: notice.org,
          examDate: notice.details?.examDate || 'Scheduled Examination',
          examCity: 'All India Designated Centres',
        ...
        if (onAddAnswerKey) {
          const keyToAdd: AnswerKey = notice.answerKeyData || {
            id: `upsc-key-${Date.now()}`,
            title: notice.title,
            org: notice.org,
            released: notice.publishedDate,
            objectionsLimit: 'Official Verified Key',
            pdfUrl: notice.pdfUrl || notice.officialUrl
          };
      ...
      localStorage.setItem('sarkari_upsc_imported_ids', JSON.stringify(updated));

      triggerToast(
        locale === 'hi'
          ? `✅ [UPSC.GOV.IN] ${notice.title} तुरंत वेबसाइट पर जोड़ दी गई!`
          : `✅ [UPSC.GOV.IN] ${notice.title} added and updated live on your website!`
      );
    } catch (e) {
      console.error('Failed to import UPSC notice:', e);
  ...
  const handleImportAllNew = () => {
    ...
    if (count > 0) {
      triggerToast(`🎉 Successfully imported ${count} fresh releases from https://www.upsc.gov.in/ to your website!`);
    } else {
      triggerToast(locale === 'hi' ? 'सभी नवीनतम UPSC सूचनाएं पहले से ही आपकी वेबसाइट पर मौजूद हैं।' : 'All latest UPSC notices are already published on your website.');
    }
  };
  ...
  const handleCustomNoticeSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('/api/upsc/auto-parse', {
        method: 'POST',
      ...
      if (res.ok) {
        ...
        if (data.notice) {
          ...
          setCustomNoticeText('');
          triggerToast(locale === 'hi' ? '✨ UPSC नोटिस का विश्लेषण कर वेबसाइट पर प्रकाशित कर दिया गया!' : '✨ UPSC Notice parsed and published live to website!');
        }
      } else {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-[#1a1c29] via-[#1e293b] to-[#0f172a] p-6 sm:p-8 text-white shadow-xl border border-slate-700">
        <div className="absolute top-0 right-0 -mt-10 -mr-10 h-64 w-64 rounded-full bg-amber-500/10 blur-3xl pointer-events-none"></div>
        <div className="absolute bottom-0 left-1/3 -mb-10 h-48 w-48 rounded-full bg-blue-500/10 blur-2xl pointer-events-none"></div>
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div className="space-y-3 max-w-2xl">
            <div className="flex flex-wrap items-center gap-2.5">
              <span className="inline-flex items-center gap-1.5 rounded-full bg-amber-500/20 px-3 py-1 text-xs font-bold text-amber-300 border border-amber-400/30">
                ...
                24x7 UPSC LIVE MONITOR ACTIVE
              </span>
              <span className="text-[11px] font-mono text-slate-200 bg-slate-800/80 px-2.5 py-0.5 rounded-md border border-slate-700">
                PORTAL: <strong className="text-white">https://www.upsc.gov.in/</strong>
              </span>
            </div>

            <h2 className="text-xl sm:text-2xl font-black tracking-tight text-white">
              {locale === 'hi' 
                ? 'संघ लोक सेवा आयोग (UPSC) त्वरित ऑटो-अपडेट इंजन' 
                : 'Union Public Service Commission (UPSC) Instant Auto-Update Engine'}
            </h2>
            <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">
              {locale === 'hi'
                ? 'जैसे ही https://www.upsc.gov.in/ या upsconline.nic.in पर सिविल सेवा (IAS/IPS), NDA, CDS, CAPF, ESE की कोई भी भर्ती, ई-प्रवेश पत्र, अंतिम मेरिट सूची या उत्तर कुंजी जारी होती है, यह सिस्टम उसे स्वतः आपकी वेबसाइट पर लाइव कर देता है।'
                : 'Monitors official releases from https://www.upsc.gov.in/ in real-time. Instantly captures Civil Services (IAS/IPS/IFS), NDA, CDS, CAPF, and ESE Vacancies, e-Admit Cards, Final Results, and Answer Keys.'}
            </p>

            {/* LIVE METRICS PILLS */}
            <div className="flex flex-wrap items-center gap-3 pt-2 text-xs font-sans">
              <div className="bg-slate-900/80 backdrop-blur-xs px-3 py-1.5 rounded-xl border border-white/10 flex items-center gap-2">
                <Globe className="h-3.5 w-3.5 text-amber-400" />
                <span className="text-slate-300">Ping Latency:</span>
                <span className="font-mono font-bold text-amber-300">{status.latencyMs}ms</span>
              </div>
              <div className="bg-slate-900/80 backdrop-blur-xs px-3 py-1.5 rounded-xl border border-white/10 flex items-center gap-2">
              ...
              <div className="bg-slate-900/80 backdrop-blur-xs px-3 py-1.5 rounded-xl border border-white/10 flex items-center gap-2">
                ...
                <span className="text-slate-300">UPSC Portal State:</span>
          ...
          <div className="flex flex-col sm:flex-row md:flex-col gap-3 shrink-0">
            ...
            >
              ...
              <span>{locale === 'hi' ? '⚡ अभी UPSC.GOV.IN से सिंक करें' : '⚡ Sync with upsc.gov.in Now'}</span>
            </button>

            <button
              onClick={handleImportAllNew}
              className="px-5 py-2.5 rounded-2xl bg-slate-700 hover:bg-slate-600 text-white font-bold text-xs transition border border-slate-600 flex items-center justify-center gap-2 cursor-pointer shadow-md"
            >
              <Sparkles className="h-4 w-4 text-amber-300" />
              <span>{locale === 'hi' ? 'सभी नई UPSC सूचनाएं वेबसाइट पर जोड़ें' : 'Import All New to Website'}</span>
            </button>
            ...
            >
              ...
              <span>Visit Official UPSC Portal (upsc.gov.in)</span>
            </a>
          </div>
        </div>
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-200 pb-3">
        <div className="flex flex-wrap gap-2">
          {[
            { id: 'all', label: 'All UPSC Releases (सभी)', icon: Zap, count: notices.length },
            { id: 'vacancy', label: 'Examinations / Vacancies (भर्तियां)', icon: Briefcase, count: notices.filter(n => n.category === 'vacancy').length },
            { id: 'admit-card', label: 'e-Admit Cards / Summon (प्रवेश पत्र)', icon: FileText, count: notices.filter(n => n.category === 'admit-card').length },
            { id: 'result', label: 'Final Results / Merit (परिणाम)', icon: Award, count: notices.filter(n => n.category === 'result').length },
        ...
        <div className="text-xs text-slate-500 font-mono">
          Showing <strong>{filteredNotices.length}</strong> official UPSC releases
        </div>
      </div>
      ...
      {loading ? (
        <div className="py-16 text-center space-y-3">
          ...
          <p className="text-xs font-bold text-slate-500">Connecting to https://www.upsc.gov.in/ live data stream...</p>
        </div>
        ...
        <div className="rounded-2xl border border-dashed border-slate-300 p-8 text-center bg-white space-y-2">
          ...
          <p className="text-xs text-slate-400">Try switching to 'All UPSC Releases' or click Sync Now.</p>
        </div>
      ) : (
          ...
          {filteredNotices.map((notice) => {
            ...
            const categoryTheme = 
              notice.category === 'vacancy' 
                ? { bg: 'bg-amber-50', text: 'text-amber-800', border: 'border-amber-200', badge: 'bg-amber-600' }
            ...
            return (
              <div
                ...
                className={`rounded-2xl border bg-white p-5 transition-all duration-200 hover:shadow-md flex flex-col justify-between ${
                  isImported ? 'border-emerald-300 ring-1 ring-emerald-200 bg-emerald-50/10' : 'border-slate-200 hover:border-amber-300'
                }`}
              >
                  ...
                  <div className="bg-slate-50 rounded-xl p-3 border border-slate-100 text-xs space-y-1.5 font-sans">
                    {notice.details?.posts && (
                      <div className="flex justify-between">
                        ...
                        <span className="font-bold font-mono text-amber-800">{Number(notice.details.posts).toLocaleString()} Posts</span>
                      </div>
                    )}
                    ...
                    {notice.details?.examDate && (
                      <div className="flex justify-between">
                        ...
                        <span className="font-bold text-blue-700">{notice.details.examDate}</span>
                      </div>
                    )}
                    {notice.details?.cutoff && (
                      <div className="flex justify-between">
                        <span className="text-slate-500">Cut-Off / Merit:</span>
                ...
                <div className="mt-4 pt-3 border-t border-slate-100 flex flex-wrap items-center justify-between gap-2">
                  <div className="flex items-center gap-2">
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="inline-flex items-center gap-1 text-[11px] font-bold text-slate-600 hover:text-amber-700 bg-slate-100 hover:bg-slate-200 px-3 py-1.5 rounded-lg transition"
                    >
                      <ExternalLink className="h-3 w-3" />
                      <span>Official PDF / Circular</span>
                    </a>
                    <a
                      href="https://www.upsc.gov.in/"
                      target="_blank"
                  ...
                  <button
                    ...
                    className={`px-4 py-2 rounded-xl text-xs font-black transition flex items-center gap-1.5 cursor-pointer ${
                      isImported
                        ...
                        : 'bg-slate-900 hover:bg-slate-800 text-amber-300 shadow-sm hover:shadow-md'
                    }`}
                  >
                    {isImported ? (
                      <>
      ...
      {/* QUICK NOTICE PARSER BOX (FOR ANY NEW UNANNOUNCED UPSC NOTICE) */}
      <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-xs space-y-4">
        <div className="flex items-center gap-2 border-b border-slate-100 pb-3">
          <Sparkles className="h-5 w-5 text-amber-500" />
          <div>
            <h3 className="text-sm font-extrabold text-slate-900">
              {locale === 'hi' ? 'त्वरित UPSC नोटिस पार्सर (Instant Custom UPSC Notice Publisher)' : 'Quick UPSC Notice Auto-Parser & Publisher'}
            </h3>
            <p className="text-xs text-slate-500">
              {locale === 'hi' 
                ? 'यदि upsc.gov.in पर कोई नया परीक्षा सर्कुलर, परिणाम या ई-प्रवेश पत्र जारी हुआ है, तो उसका विवरण या लिंक यहाँ पेस्ट करें। AI उसे तुरंत वेबसाइट पर प्रकाशित कर देगा।'
                : 'Paste any fresh notice text, gazette notification, or circular from https://www.upsc.gov.in/. The engine will auto-categorize and immediately publish it live to your website.'}
            </p>
          </div>
        </div>
        ...
        <form onSubmit={handleCustomNoticeSubmit} className="space-y-3">
          <textarea
            ...
            rows={3}
            placeholder="e.g. UPSC Combined Geo-Scientist Examination 2026 Notification released for 85 Geologist, Geophysicist & Chemist posts at upsc.gov.in..."
            className="w-full rounded-xl border border-slate-200 p-3 text-xs text-slate-800 focus:border-amber-500 focus:outline-hidden font-sans"
            disabled={isParsingNotice}
          />

          <div className="flex items-center justify-between gap-3">
            <span className="text-[11px] text-slate-400 font-mono">
              Source verified: https://www.upsc.gov.in/
            </span>

            <button
              type="submit"
              disabled={isParsingNotice || !customNoticeText.trim()}
              className="px-5 py-2.5 rounded-xl bg-slate-900 hover:bg-slate-800 text-amber-300 text-xs font-black transition flex items-center gap-2 cursor-pointer shadow-sm disabled:opacity-50"
            >
              {isParsingNotice ? (
                <>
